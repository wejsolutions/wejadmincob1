...
<?php foreach($mclientecuenta->cuentas_poride($ide) as $r): ?>
	<form action="" class="op_elimina form-horizontal">
		<?php echo $fn->modalHeader('Borrar Cuenta Bancaria') ?>
		<div class="modal-body">
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
					<input type="text" class="form-control" name="des" value="<?php echo $r->ctecue_cuenta ?>" disabled>
				</div>
			</div>
		</div>
	...
	$(function(){
		...
		$(formulario).validate({
			...
			submitHandler: function (form) {
				$.post('prc-mclientecuenta-delete_cuenta',$(formulario).serialize(),function(data){
					if(data==1) {
						load('vst-clientecuenta-lista','','.cuentas');
