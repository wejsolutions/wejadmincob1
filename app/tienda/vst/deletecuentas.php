...
<?php foreach($mtienda->poridecuenta($ide) as $r): ?>
	<form action="" class="op2 form-horizontal">
		<?php echo $fn->modalHeader('Borrar Cuenta');
		...
		<div class="modal-body">
			...
			<div class="form-group">
				...
				<div class="col-sm-4">
					<input type="text" class="form-control" name="cue" value="<?php echo $r->empcue_cuenta ?>" readonly="true">
				</div>
			</div>
			<div class="form-group">
				<label for="" class="control-label col-sm-2 bolder">Código:</label>
				<div class="col-sm-4">
					<input type="text" class="form-control" name="cod" value="<?php echo $r->empcue_codigo ?>" readonly="true">
				</div>
				<label for="" class="control-label col-sm-2 bolder">Titular:</label>
				<div class="col-sm-4">
					<input type="text" class="form-control" name="tit" value="<?php echo $r->empcue_nombre ?>" readonly="true">
				</div>
			</div>
			<div class="form-group">
				<label for="" class="control-label col-sm-2 bolder">Email:</label>
				<div class="col-sm-4">
					<input type="email" class="form-control" name="cor" value="<?php echo $r->empcue_email ?>" readonly="true">
				</div>
			</div>
		</div>
	...
	$(function(){
		...
		$(formulario).validate({
			...
			submitHandler: function (form) {
				$.post('prc-mtienda-deletecuenta',$(formulario).serialize(),function(data){
					if(data==1) {
